...
def main():
    ...
    for epoch in range(N_EPOCHS):
        if epoch == 0:
            if pretrain_G_first:
                for x in range(TRAIN_G_FIRST):
                    ...
                    validate(data_loader=val_loader, model_G=G, model_D=D, criterion=criterion, epoch=epoch)
        ...
        if VALIDATE:
            val_D_fake_loss, val_D_real_loss, val_G_loss, epoch_diff = validate(data_loader=val_loader, model_G=G,
                                                                                model_D=D,
