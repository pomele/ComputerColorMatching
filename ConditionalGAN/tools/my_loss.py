...
class ColorDiffLoss(loss.Module):
    def __init__(self, BATCHSIZE, model):
        super(ColorDiffLoss, self).__init__()
        self.n_grid = BATCHSIZE
        self.model = model

    def forward(self, concentration_input, target_reflectance):
        return get_lik(concentration_input, target_reflectance, n_grid=self.n_grid, model=self.model)
...
initial_concentration = torch.Tensor([0.51, 0.51, 0.51, 0.51, 0.51, 0.51])
background = torch.Tensor(
    [0.4519, 0.7445, 0.8898, 0.9311, 0.9374, 0.9396, 0.9426, 0.9435, 0.9453, 0.9456, 0.9472, 0.9475,
...
perfect_white = torch.Tensor([[94.83], [100.00], [107.38]])
...
def get_lik(concentration, reflectance, n_grid=BATCH_SIZE, model='km', sigma=None, xvec=None,
    ...
    diff = torch.zeros(n_grid).to(device)
    if model == 'km':
        fsb = ((torch.ones_like(background) - background) ** 2 / (background * 2))
        # 这里有0
        fst = (((torch.ones_like(ingredients) - ingredients) ** 2 / (ingredients * 2) - fsb) / init_conc_array)
...
def color_diff(reflectance1, reflectance2):
    # print(type(optical_relevant))
    # print(type(reflectance1.reshape(31, 1)))
    global optical_relevant
    if torch.cuda.is_available():
        device = 'cuda'
    else:
        device = 'cpu'

    optical_relevant = optical_relevant.to(device)
    tri1 = torch.mm(optical_relevant, reflectance1.reshape(31, 1))
    tri2 = torch.mm(optical_relevant, reflectance2.reshape(31, 1))
...
def xyz2lab(xyz):
    r = 0.008856
    global perfect_white
    if torch.cuda.is_available():
        device = 'cuda'
    else:
        device = 'cpu'
    lab = torch.zeros(3, 1).to(device)
    perfect_white = perfect_white.to(device)
